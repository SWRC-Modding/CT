...
template<typename T>
inline T* New(FMemStack& Mem, INT Count = 1, INT Align = DEFAULT_ALIGNMENT){
...
template<typename T>
inline T* NewZeroed(FMemStack& Mem, INT Count = 1, INT Align = DEFAULT_ALIGNMENT){
	guardSlow(FMemStack::New);
...
template<typename T>
inline T* NewOned(FMemStack& Mem, INT Count = 1, INT Align = DEFAULT_ALIGNMENT){
	guardSlow(FMemStack::New);
	BYTE* Result = Mem.PushBytes(Count * sizeof(T), Align);
